...
C      IF(NASER.GT.0)THEN
        ...
        if (n.eq.1) then
        OPEN(1,FILE='ADIAG.OUT',STATUS='UNKNOWN') 
        CLOSE(1,STATUS='DELETE')  
        end if
        OPEN(1,FILE='ADIAG.OUT',STATUS='UNKNOWN',position='append')  
        write(1,*) 'n =', n, timeday 
          DO NA=1,NASER  
            WRITE(1,*) NA,PATMTT(NA), TATMTT(NA), TWETTT(NA)   
            WRITE(1,*) RAINTT(NA), EVAPTT(NA) 
            WRITE(1,*) SOLSWRTT(NA), CLOUDTT(NA)    
          ENDDO
        CLOSE(1)  
        ENDIF  
