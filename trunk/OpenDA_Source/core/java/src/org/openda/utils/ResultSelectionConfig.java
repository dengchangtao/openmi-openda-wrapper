...
public class ResultSelectionConfig {
	...
	public void addResultItem(String id, int minSize, int maxSize, String outputLevel, String context, boolean doLog) {
		String key = composeHashKey(id, minSize, maxSize, outputLevel, context);
	...
	public int nextWriteCounter(String id, int size, String outputLevel, String context, IResultWriter.MessageType messageType) {
		String key = getMatchingKey(id,size,context,outputLevel);
	...
	private String getMatchingKey(String id, int size, String context, String outputLevel) {
		String defaultId = "any";
		int defaultMinSize = 1;
		int defaultMaxSize = Integer.MAX_VALUE;
		String defaultOutputLevel = "all";
		...
		for (int i=0; i<keySet.length; i++){
			...
			boolean idMatched = itemAttributes[indexId].equalsIgnoreCase(defaultId) || itemAttributes[indexId].equalsIgnoreCase(id);
			...
			boolean sizeMatched = (specifiedMaxSize == defaultMaxSize || size <= specifiedMaxSize) &&
					(specifiedMinSize == defaultMinSize || size >= specifiedMinSize);

			int outLevelSpecified = getOutLevelInteger(itemAttributes[indexOutputLevel]);
			int outLevelThisItem = getOutLevelInteger(outputLevel);
			boolean outputLevelMatched = outLevelThisItem <= outLevelSpecified;

			boolean contextMatched = itemAttributes[indexContext].matches(defaultContext) || context.matches(defaultContext) || context.matches(itemAttributes[indexContext]);
	...
	private int getOutLevelInteger(String outputLevel) {
		int outputLevelNone = 0;
		int outputLevelEssential = 1;
		int outputLevelNormal = 2;
		int outputLevelVerbose = 3;
		int outputLevelAll = 4;

		if (outputLevel.equalsIgnoreCase("none")){
			return outputLevelNone;
		} else if (outputLevel.equalsIgnoreCase("essential")){
			return outputLevelEssential;
		} else if (outputLevel.equalsIgnoreCase("normal")){
			return outputLevelNormal;
		} else if (outputLevel.equalsIgnoreCase("verbose")){
			return outputLevelVerbose;
		} else {
			return outputLevelAll;
		}
	}
