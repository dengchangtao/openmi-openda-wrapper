 ...
 * This module is intended for coupling to another model, usualy to extend a physical model
...
public class MapsNoiseModelInstance extends Instance implements IStochModelInstance {
	...
	protected HashMap<String, Integer> mSize=new HashMap<String, Integer>();
	protected HashMap<String, Integer> nSize=new HashMap<String, Integer>();
	...
	public void initialize(File workingDir, String[] arguments) {
		...
		for(int i=0;i<itemTrees.length;i++){
			...
			this.mSize.put(id, x.length);
			this.nSize.put(id, y.length);
			...
			SpatialCorrelationStochVector systemNoisePart = 
	...
	public void compute(ITime targetTime) {
		...
		for(int itemIndex=0;itemIndex<this.ids.size();itemIndex++){
			...
			int m=this.mSize.get(ids.get(itemIndex));
			int n=this.nSize.get(ids.get(itemIndex));
			allValues[itemIndex] = new Array(new int[]{nsteps+1,m,n});
		}
		...
		for(int i=0;i<nsteps;i++){
			...
			if(this.outputLevel!=OutputLevel.Suppress){
				Results.putValue("model_time", this.t, org.openda.interfaces.IResultWriter.MessageType.Step);
				Results.putValue("x_noise_series", x, org.openda.interfaces.IResultWriter.MessageType.Step);
			}
	...
	public class savedState implements IModelState {
		...
		public void savePersistentState(File file) {
			try {
				...
				tempVec.maxFullExpandLength = this.state.getSize()+1;
	...
	private void expandGrid(double x[],double y[],double x2[],double y2[]){
		int index=0;
