...
public class ResultWriterTest extends TestCase {
    ...
    private void doAlgorithmSteps() {
        ...
        Results.putValue(IResultWriter.Source.Algorithm,"treevector", v2);
        ...
        for (int i = 0; i < maxStep; i++) {
            ...
            Results.putValue("v1", nextValues(v), IResultWriter.MessageType.Step);
            Results.putValue(IResultWriter.Source.Algorithm, "someValues", nextValues(v));
            for (int j = 0; j < maxOuterIter; j++) {
                ...
                for (int k = 0; k < maxInnerIter; k++) {
                    ...
                    Results.putValue("modelValues", nextValues(v), IResultWriter.MessageType.InnerIteration);
                    Results.putValue(IResultWriter.Source.Model, "someValues", nextValues(v));
                    Results.putValue("costEvaluation", nextValues(v));
                }
                Results.putValue("Outer result", nextValues(v));
            }
            Object dummy = null;
    ...
    private void doAlgorithmMcSteps() {
        ...
        for (int i = 0; i < maxStep; i++) {
            ...
            Results.putValue("evaluatedParameters",params,IResultWriter.MessageType.Instance);
            Results.putValue("residuals", v2, IResultWriter.MessageType.Instance);
        }
    }
    ...
    private void doAlgorithmStepsLevels() {
        ...
        for (int i = 0; i < maxStep; i++) {
            ...
            Results.putValue(testAlgorithm, "someValues", nextValues(v));
            ...
            for (int j = 0; j < maxOuterIter; j++) {
                ...
                for (int k = 0; k < maxInnerIter; k++) {
                    ...
                    Results.putValue(testInnerLoop, "modelValues", nextValues(v));
                    Results.putValue(testInnerLoop, "someValues", nextValues(v));
                    Results.putValue(testInnerLoop, "costEvaluation", nextValues(v));
                }
                Results.putValue(testOuterLoop, "Outer result", nextValues(v));
            }
        }
    }
