/*
$URL: https://repos.deltares.nl/repos/openda/openda_1/public/trunk/core/native/src/cta/cta_treevector.c $
$Revision: 3734 $, $Date: 2012-12-19 12:37:25 +0100 (wo, 19 dec 2012) $
...
int CTAI_TreeVec_GetSetVal(CTA_TreeVector treevec1, int i, void *val,
   ...
   if (data1->nsubtrees==0){
      ...
      if (data1->v) {
         ...
         if (retval!=CTA_OK) {
	        CTA_WRITE_ERROR("Error in getting or setting vals");
	        return retval;
         }
      }
   }
...
// DIT IS EEN ERG KORT DOOR DE BOCHT VERSIE'
   ...
   if (retval!=CTA_OK) {
       CTA_WRITE_ERROR("Cannot get vector from treevector");
       return retval;
   }
