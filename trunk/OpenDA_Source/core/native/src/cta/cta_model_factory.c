/*
$URL: https://repos.deltares.nl/repos/openda/openda_1/public/trunk/core/native/src/cta/cta_model_factory.c $
$Revision: 3570 $, $Date: 2012-10-10 15:21:41 +0200 (wo, 10 okt 2012) $
...
double *ChildComputeEndTime; /* End time of all blocked child models (note: single value for all models)                */
double *ChildComputeCurTime; /* Current model time at block of model (note: single value for all models)                */
} CTAI_ModelClass;
...
int CTAI_Model_DuplicateClass(CTA_ModelClass hmodcl,
   ...
   double *ChildComputeEndTime, *ChildComputeCurTime;
   int retval;
   int i;
   ...
   clsdata_dup->ChildComputeCurTime   = NULL;
   clsdata_dup->ChildComputeEndTime   = NULL;
   ...
   ChildComputeEndTime = clsdata->ChildComputeEndTime;
   if (ChildComputeEndTime) {
      clsdata_dup->ChildComputeEndTime = ChildComputeEndTime;
   }

   ChildComputeCurTime = clsdata->ChildComputeCurTime;
   if (ChildComputeCurTime) {
      clsdata_dup->ChildComputeCurTime = ChildComputeCurTime;
   }
...
int CTAI_ModelFac_SetBlock(CTA_ModelClass hmodcl, CTA_Model hmodel, double tstart, double tstop){
   ...
   for (imodel=0;imodel<clsdata->nChildModels;imodel++){
      if (clsdata->ChildModels[imodel]==hmodel){
         ...
         if (clsdata->ChildComputeCurTime==NULL) {
           clsdata->ChildComputeCurTime = CTA_Malloc(sizeof(double));
           *clsdata->ChildComputeCurTime = tstart;
         }
         eps=M_EPS*fabs(tstart)+M_EPS;
         if (fabs(*clsdata->ChildComputeCurTime - tstart) > eps ){
            printf("Error in CTA_ModelFac_SetBlock: unequal start times compute interval\n");
            printf("Error in CTA_ModelFac_SetBlock: tstart=%f, currTime=%f, eps=%f\n",tstart, *clsdata->ChildComputeCurTime, eps );
            exit(-1);
         }

         if (clsdata->ChildComputeEndTime==NULL) {
            clsdata->ChildComputeEndTime = CTA_Malloc(sizeof(double));
            *clsdata->ChildComputeEndTime = tstop;
         }
         eps=M_EPS*fabs(tstop)+M_EPS;
         if (fabs(*clsdata->ChildComputeEndTime - tstop) > eps ){
            printf("Error in CTA_ModelFac_SetBlock: unequal end times compute interval");
            printf("Error in CTA_ModelFac_SetBlock: tstop=%f, endTime=%f, eps=%f\n",tstop, *clsdata->ChildComputeEndTime, eps );
            exit(-1);
         }
      }
   }
...
int CTAI_ModelFac_TimeStepAllModels(CTA_ModelClass hmodcl, CTA_Function *function){
   ...
   if (nblock==clsdata->nChildModels) {
      ...
      nStep=(int) ((*clsdata->ChildComputeEndTime-*clsdata->ChildComputeCurTime)/tstep + 0.5);
      ...
      t1 = *clsdata->ChildComputeCurTime;
      for (istep=0; istep < nStep; istep++){
         t2 = MIN(t1 + tstep,*clsdata->ChildComputeEndTime);
...
int CTA_Model_DefineClass(
   ...
   data->ChildComputeCurTime   = NULL;
   data->ChildComputeEndTime   = NULL;
