!
! $URL: https://repos.deltares.nl/repos/openda/openda_1/public/trunk/core/native/examples/heat_stoch/src/cta_model.f $
! $Revision: 2451 $, $Date: 2011-06-18 20:29:19 +0200 (za, 18 jun 2011) $
!
...
!     deterministic part and the noise part
     ...
     +   'diff_stoch', datblc(INDX_STATE),retval)
      if (retval.ne.CTA_OK) 
...
!     next the handle to the heat noise substate
      call cta_treevector_getsubtreevec(hnoise, 'diff_nheat',  hnheat,
     +   retval)
      if (retval.ne.CTA_OK) 
     +           stop 'heatstoch_compute: Error getting hnheat'
!     next the handle to the boundary condition noise substate
      call cta_treevector_getsubtreevec(hnoise, 'diff_nbc',    hnbc  ,
     +   retval)
      if (retval.ne.CTA_OK) 
