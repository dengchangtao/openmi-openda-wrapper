...
public class CostTest extends TestCase {
	...
	public void testClonedCostFunction() {
		...
		double J1Init = J1.evaluate(p1Init);
		double J2Init = J2.evaluate(p2Init);
		double J1True = J1.evaluate(p1True);
		double J2True = J2.evaluate(p2True);
	...
	public void testSimpleCost() {
		...
		double value1 = cost.evaluate(new Vector("[1.0,1.0]"));
		assertEquals(0.0, value1, delta);
		double value2 = cost.evaluate(new Vector("[2.0,2.0]"));
