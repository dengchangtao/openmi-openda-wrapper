...
public class EnSR extends AbstractSequentialEnsembleAlgorithm {
	...
	public void analysis(IStochObserver obs, IVector obsValues, IVector predMainModel, 
		...
		Results.putValue("std_x_f", xiStd, IResultWriter.MessageType.Step);
		...
		//Results.putValue("obs", obsVal, IResultWriter.MessageType.Step);
		//
		// compute Kalman gain
		// K = P*H'/(H*P*H'+R)
		...
		Results.putValue("sqrt_r", sqrtR, IResultWriter.MessageType.Step);
		phi.multiply(1.0, sqrtR, sqrtR, 1.0, false, true); // phi+=R
		Results.putValue("hpht_plus_r", phi, IResultWriter.MessageType.Step);
		// K = L*(HL)'*inv(phi) = XI * W
		...
		Results.putValue("L_f", LfMat, IResultWriter.MessageType.Step);
		Matrix K = new Matrix(n,m);
		K.multiply(1.0, LfMat,W, 0.0, false, false);
		Results.putValue("k", K, IResultWriter.MessageType.Step);
		...
		Results.putValue("x_a_linear", xiNewAvg, IResultWriter.MessageType.Step);
		...
		*/
		...
		//Results.putValue("psi", psi, ResultWriter.MessageType.Step);
		// chi = ( I - psi'*psi )
		Matrix chi = Matrix.eye(q);
		chi.multiply(-1.0,psi, psi, 1.0,true,false);
		Matrix[] svdDecomposition = chi.svd();
		//Results.putValue("chi", chi, ResultWriter.MessageType.Step);
		// U*D*V' = chi
		Matrix U = svdDecomposition[0];
		Matrix D = svdDecomposition[1];
		Matrix V = svdDecomposition[2];
		//Results.putValue("U", U, ResultWriter.MessageType.Step);
		//Results.putValue("D", D, ResultWriter.MessageType.Step);
		//Results.putValue("V", V, ResultWriter.MessageType.Step);
        ...
        //Results.putValue("Ts", Ts, ResultWriter.MessageType.Step);
        ...
        Results.putValue("L_a", new Matrix(L_a), IResultWriter.MessageType.Step);
        for(int i=0;i<q;i++){
		...
		//Results.putValue("x_a", xiAvg, IResultWriter.MessageType.Step);
		Results.putValue("std_x_a", xiStd, IResultWriter.MessageType.Step);
		predAvg = ensembleAverage(pred);
		//Results.putValue("pred_a", predAvg, IResultWriter.MessageType.Step);
	}

	public IModelState loadPersistentState(File algorithmStateFile) {
		throw new UnsupportedOperationException("org.openda.algorithms.kalmanFilter.EnSR.loadPersistentState(): Not implemented yet.");
	}
}
