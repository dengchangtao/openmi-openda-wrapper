...
public class WflowPythonToJavaAdapterTest extends TestCase {
	 ...
	 * PYTHONPATH: folder with WFLOW python scripts (e.g. opendaTestRuns\model_wflow\org\openda\model_wflow\wflow_bin\) and folder with PCRaster python scripts (e.g. C:\PCRaster\Python).
	 *
	 * Manual test to test the WflowPythonToJavaAdapter class.
	 *
	 * @throws JepException
	 */
	public void manualTest() throws JepException {
		...
		long timeStepLengthInMillies = 24*3600*1000;
		//working directory (testRunDataDir) is opendaTestRuns/model_wflow/org/openda/model_wflow/
		File outMapsDir = new File(caseDir, runId + "/outmaps");
		adapter.writePiMapStackXmlFile(new File(outMapsDir, "lev.xml"), "lev?????.???", "lev", "lev", timeHorizon, timeStepLengthInMillies);
		adapter.runInitial();
		adapter.runResume();
		...
		File actualOutputFile2 = new File(testRunDataDir, "wflowPythonToJavaAdapterTest/case_default_hbv/work0/outmaps/lev.xml");
		File expectedOutputFile2 = new File(testRunDataDir, "wflowPythonToJavaAdapterTest/expectedResult/lev.xml");
		assertTrue(testData.FilesAreIdentical(expectedOutputFile2, actualOutputFile2, 0));
	}
	 ...
	 */
	public void manualTestMemoryUsage() throws JepException, InterruptedException {
		Calendar calendar = Calendar.getInstance();
		calendar.setTimeZone(TimeUtils.createTimeZoneFromDouble(0));
		calendar.set(2012, 0, 1, 0, 0, 0);
		calendar.set(Calendar.MILLISECOND, 0);
