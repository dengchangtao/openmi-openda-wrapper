...
public class SwanOpenBoundaryTPARFile implements IDataObject, ITimeInfo {
    ...
    private OpenBoundaryTPAR openBoundaryData;
    ...
    protected String[] getTimeStr(){
        return this.openBoundaryData.time;
    }
    ...
    protected double[] getHs(){
        double[] Hs = new double[this.openBoundaryData.Hs.length];
        System.arraycopy(this.openBoundaryData.Hs,0,Hs,0,Hs.length);
        return Hs;
    }

    protected void setHs(double[] Hs){
        System.arraycopy(Hs,0,this.openBoundaryData.Hs,0,Hs.length);
    }
    ...
    protected double[] getPeriod(){
        double[] period = new double[this.openBoundaryData.period.length];
        System.arraycopy(this.openBoundaryData.period,0,period,0,period.length);
        return period;
    }

    protected void setPeriod(double[] period){
        System.arraycopy(period,0,this.openBoundaryData.period,0,period.length);
    }
    ...
    protected double[] getPeakDirection(){
        double[] peakDirection = new double[this.openBoundaryData.peakDirection.length];
        System.arraycopy(this.openBoundaryData.peakDirection,0,peakDirection,0,peakDirection.length);
        return peakDirection;
    }

    protected void setPeakDirection(double[] peakDirection){
        System.arraycopy(peakDirection,0,this.openBoundaryData.peakDirection,0,peakDirection.length);
    }
    ...
    protected double[] getDirectSpread(){
        double[] directSpread = new double[this.openBoundaryData.directSpread.length];
        System.arraycopy(this.openBoundaryData.directSpread,0,directSpread,0,directSpread.length);
        return directSpread;
    }

    protected void setDirectSpread(double[] directSpread){
        System.arraycopy(directSpread,0,this.openBoundaryData.directSpread,0,directSpread.length);
    }
    ...
    protected OpenBoundaryTPAR getOpenBoundary(){
        return this.openBoundaryData;
    }

    protected void setOpenBoundary(OpenBoundaryTPAR openBoundaryData){
        int length = this.openBoundaryData.time.length;
        System.arraycopy(openBoundaryData.getTime(),0,this.openBoundaryData.time,0,length);
        System.arraycopy(openBoundaryData.getHs(),0,this.openBoundaryData.Hs,0,length);
        System.arraycopy(openBoundaryData.getPeriod(),0,this.openBoundaryData.period,0,length);
        System.arraycopy(openBoundaryData.getPeakDirection(),0,this.openBoundaryData.peakDirection,0,length);
        System.arraycopy(openBoundaryData.getDirectSpread(),0,this.openBoundaryData.directSpread,0,length);
    ...
    private void writeOpenBoundaryFile(){
        ...
        try {
            ...
            for (int i=0;i<this.openBoundaryData.nTimeLevel;i++) {
                ...
                //under linux.
                outputFileBufferedWriter.write(" "+this.openBoundaryData.time[i]);
                outputFileBufferedWriter.write(String.format(locale, openBoundaryFormat,this.openBoundaryData.Hs[i]));
                outputFileBufferedWriter.write(String.format(locale, openBoundaryFormat,this.openBoundaryData.period[i]));
                outputFileBufferedWriter.write(String.format(locale, openBoundaryFormat,this.openBoundaryData.peakDirection[i]));
                outputFileBufferedWriter.write(String.format(locale, openBoundaryFormat,this.openBoundaryData.directSpread[i]));
                outputFileBufferedWriter.newLine();
            }
    ...
    private void readOpenBoundaryFile(){
            ...
            openBoundaryData = new OpenBoundaryTPAR(nTimeLevel,time,Hs,period,peakDirection,directSpread);
    }
	...
	private class OpenBoundaryTPAR {
        public int nTimeLevel;
        public String[] time;
        public double[] Hs;
        public double[] period;
        public double[] peakDirection;
        public double[] directSpread;

        public OpenBoundaryTPAR(int nTimeLevel, String[] time, double[] Hs, double[] period, double[] peakDirection, double[] directSpread){

            this.nTimeLevel = nTimeLevel;
            this.time = new String[nTimeLevel];
            this.Hs = new double[nTimeLevel];
            this.period = new double[nTimeLevel];
            this.peakDirection = new double[nTimeLevel];
            this.directSpread = new double[nTimeLevel];

            for (int i=0;i<nTimeLevel;i++){
                this.time[i] = time[i];
                this.Hs[i] = Hs[i];
                this.period[i] = period[i];
                this.peakDirection[i] = peakDirection[i];
                this.directSpread[i] = directSpread[i];
            }
        }
