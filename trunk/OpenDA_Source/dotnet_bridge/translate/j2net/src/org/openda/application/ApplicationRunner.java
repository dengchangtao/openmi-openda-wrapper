...
public class ApplicationRunner implements Runnable{
	...
	private int currenAlgorithmStep = 0;
	...
	protected IAlgorithm startApplication(File workingDir, String fileName) {
		return startApplication(workingDir, fileName, null);
	}
	...
	private IAlgorithm createApplicationComponents(File applicationConfigFile, ITime timeHorizon) {
		...
		IAlgorithm algorithm = ObjectSupport.createAlgorithm(configuration.getComponentConfig(OpenDaConfiguration.ALGORITHM), stochObserver, stochModelFactory);
		...
		if (writeAtThisTime){
			...
			if(time==null){
				...
				if(addRestartTimeTag){
					timeString= String.valueOf(++currenAlgorithmStep);
				}
