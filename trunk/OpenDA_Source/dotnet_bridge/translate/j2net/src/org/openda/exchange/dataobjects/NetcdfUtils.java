...
public class NetcdfUtils {
	...
	public static final float FILL_VALUE = 9.96921e+036f;
	...
	public static IArrayGeometryInfo createGeometryInfo(Variable variable, NetcdfFile netcdfFile) {
		...
		//TODO only 2D grids supported. AK
	...
	public static double[] readSelectedData(Variable variable, int[] origin, int[] sizeArray) {
	...
	public static double[] readDataForVariableForSingleLocation(Variable variable, int stationDimensionIndex, int stationIndex) {
		...
		return readSelectedData(variable, origin, sizeArray);
	}

	public static double[] readDataForVariableForSingleTime(Variable variable, int timeDimensionIndex, int timeIndex) {
		int[] origin = createOrigin(variable);
		int[] sizeArray = variable.getShape();
	...
	public static void writeDataForVariableForSingleTime(NetcdfFileWriteable netcdfFile, Variable variable, int timeIndex, double[] values) {
		int[] origin = createOrigin(variable);
		int[] sizeArray = variable.getShape();

		//select only the given time.
		//assume that timeDimensionIndex is always 0 here.
	...
	public static void createMetadata(NetcdfFileWriteable netcdfFile, IExchangeItem exchangeItem,
		...
		if (timeInfo != null && timeInfo.getTimes() != null) {//if variable depends on time.
			...
			if (timeDimension == null) {//if timeVariable with correct times not yet present.
				...
				netcdfFile.addVariableAttribute(timeVariableName, AXIS_ATTRIBUTE_NAME, "T");
		...
		if (geometryInfo != null) {//if variable depends on space.
			...
			if (geometryInfo instanceof IrregularGridGeometryInfo) {
				...
				if (gridVariableProperties != null) {
					...
					String latitudeDimensionName = LATITUDE_VARIABLE_NAME + postfix;
					String longitudeDimensionName = LONGITUDE_VARIABLE_NAME + postfix;
					...
					Dimension latitudeDimension = netcdfFile.addDimension(latitudeDimensionName, rowCount);
					Dimension longitudeDimension = netcdfFile.addDimension(longitudeDimensionName, columnCount);
					dimensions = Arrays.asList(new Dimension[]{latitudeDimension, longitudeDimension});
				...
				//TODO implement for general case. AK
			}
		}
		...
		switch (exchangeItem.getValuesType()) {
			case doubles2dType: case doublesType: case doubleType: case IArrayType: case IVectorType:
		...
		netcdfFile.addVariableAttribute(dataVariableName, STANDARD_NAME_ATTRIBUTE_NAME, dataVariableName);
		...
		netcdfFile.addVariableAttribute(dataVariableName, FILL_VALUE_ATTRIBUTE_NAME, FILL_VALUE);
		netcdfFile.addVariableAttribute(dataVariableName, MISSING_VALUE_ATTRIBUTE_NAME, FILL_VALUE);
	}
	...
	public static void writeMetadata(NetcdfFileWriteable netcdfFile, Map<ITimeInfo, Dimension> timeInfoTimeDimensionMap,
			Map<IGeometryInfo, GridVariableProperties> geometryInfoGridVariablePropertiesMap) throws Exception {
