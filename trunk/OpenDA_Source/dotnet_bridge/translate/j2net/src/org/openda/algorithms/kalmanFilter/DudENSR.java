...
public class DudENSR extends AbstractSequentialEnsembleAlgorithm {
	...
	public void analysis(IStochObserver stochObs, IVector obsValues, IVector predMainModel, 
		...
		Results.putValue("std_obs", stochObs.getStandardDeviations(), IResultWriter.MessageType.Step);
		...
		for(int j=0;j<q;j++){
			...
			initialCosts[j]  = f.evaluate(initialTransformedStates[j]);
		}
		// and p[n+1] = 0
		initialTransformedStates[q] = new Vector(q); // p=[0,...,0]
		initialCosts[q] = f.evaluate(initialTransformedStates[q]);
		...
		Results.putValue("L_a", LaMat, IResultWriter.MessageType.Step);
	}

	public IModelState loadPersistentState(File algorithmStateFile) {
		throw new UnsupportedOperationException("org.openda.algorithms.kalmanFilter.DudENSR.loadPersistentState(): Not implemented yet.");
	}
}
