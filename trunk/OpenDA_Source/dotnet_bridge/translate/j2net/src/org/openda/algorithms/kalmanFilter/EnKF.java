...
public class EnKF extends AbstractSequentialEnsembleAlgorithm {
	...
	public void analysis(IStochObserver obs, IVector obsValues, IVector predictions,
		...
		timerLinalg.stop();
		timerResults.start();
		Results.putValue("hpht_plus_r", D, IResultWriter.MessageType.Step);
		...
		for(int i=0;i<m;i++){
			...
			timerLinalg.stop();
			timerResults.start();
			Results.putValue("k_"+i, Kvecs[i], IResultWriter.MessageType.Step);
		    timerResults.stop();
		}
		...
		timerLinalg.stop();
		timerResults.start();
		Results.putValue("pred_a_linear", pred_a_linear, IResultWriter.MessageType.Step);
	    timerResults.stop();
		...
		//Results.putValue("L_a", LaMat, ResultWriter.MessageType.Step);
		//
		timerTotal.stop();
        System.gc();
	}
