...
public class OpenDaConfigurationReader {
    ...
    private OpenDaResultWriterConfig parseResultWriterConfig(File openDaApplicationFile, OpenDaResultWriterXML resultWriterXML) {
        ...
        ResultSelectionConfig resultSelectionConfig = null;
        if (resultWriterXML.getSelection() != null) {
            resultSelectionConfig = parseResultSelectionConfig(resultWriterXML.getSelection());
        }
...
//        resultSelectionConfig.setWriteNonConfiguredItems(resultSelectionXML.getDoLog().getNonConfiguredItems());
//        resultSelectionConfig.setWriteAlgorithmOutput(resultSelectionXML.getDoLog().getAlgorithm());
//        resultSelectionConfig.setWriteModelOutput(resultSelectionXML.getDoLog().getModel());
//        resultSelectionConfig.setWriteObserverOutput(resultSelectionXML.getDoLog().getObserver());
        ...
        for (ResultItemXML resultItemXML : resultItemXMLs) {
			...
			// begin JHS (07-09-2012)
			if (resultSelectionXML.getDoLog() != null) {
				// JHS: use this to keep the old format work. Here the doLog element should be specified in the xml
				// configuration file. TODO: remove this block once everything implements the new putValue.
			resultSelectionConfig.addResultItem(resultItemXML.getId(),
					sourceAsString, resultItemXML.getDoLog());
			} else {
			resultSelectionConfig.addResultItem(resultItemXML.getId(), resultItemXML.getMinSize(),
					resultItemXML.getMaxSize(),resultItemXML.getOutputLevel(),
					resultItemXML.getContext(), resultItemXML.getDoLog());
			}
			// end JHS (07-09-2012)
        }
