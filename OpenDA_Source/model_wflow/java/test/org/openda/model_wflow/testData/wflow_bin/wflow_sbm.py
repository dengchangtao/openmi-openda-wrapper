...
$Author: schelle $
$Id: wflow_sbm.py 336 2012-06-15 10:02:43Z schelle $
$Rev: 336 $
"""
import numpy
import pcrut
...
wflow = "wflow_sbm: "
wflowVersion = "$Revision: 336 $  $Date: 2012-06-15 12:02:43 +0200 (Fri, 15 Jun 2012) $"
...
class WflowModel(DynamicModel):
  ...
  def initial(self):
    ...
    self.Altitude=readmap(self.Dir + "/staticmaps/wflow_dem") * scalar(defined(subcatch)) # DEM
    self.TopoLdd=readmap(self.Dir + "/staticmaps/wflow_ldd.map")        # Local
    self.TopoId=readmap(self.Dir + "/staticmaps/wflow_subcatch.map")        # area map  
    River=cover(boolean(readmap(self.Dir + "/staticmaps/wflow_river.map")),0)
    ...
    self.LandUse=readmap(self.Dir + "/staticmaps/wflow_landuse.map")
    self.LandUse=cover(self.LandUse,nominal(ordinal(subcatch) > 0))
    self.Soil=readmap(self.Dir + "/staticmaps/wflow_soil.map")
    self.Soil=cover(self.Soil,nominal(ordinal(subcatch) > 0))
    self.OutputLoc=readmap(self.Dir + "/staticmaps/wflow_gauges.map")        # location of output gauge(s)
    self.InflowLoc=pcrut.readmapSave(self.Dir + "/staticmaps/wflow_inflow.map",0.0)  # location abstractions/inflows.
    self.SeepageLoc=pcrut.readmapSave(self.Dir + "/staticmaps/wflow_inflow.map",0.0)  # location abstractions/inflows.
    ...
    if self.scalarInput:
        self.gaugesMap=readmap(self.Dir + "/staticmaps/wflow_mgauges.map")        # location of rainfall/evap/temp gauge(s)
    ...
    self.TempCor=pcrut.readmapSave(self.Dir + "/staticmaps/" + configget(self.config,"model","TemperatureCorrectionMap","wflow_tempcor.map"),0.0)
    ...
    if self.modelSnow:
        ...
        # soil factor wigmosta et al 2009
  ...
  def dynamic(self):
    ...
    if self.scalarInput:
            ...
            self.Inflow = cover(timeinputscalar(self.inflowTss,self.InflowLoc),0)
            ...
            self.PotenEvap=timeinputscalar(self.evapTss,self.gaugesMap)
    ...
    #TDOD: add sub-daily interception!
    ##########################################################################

    #if self.timestepsecs < 86399:
    #    rainfall_interception_hbv(Rainfall,PotEvaporation,Cmax,InterceptionStorage)
    #else:
    ThroughFall, Interception, StemFlow = pcrut.rainfall_interception_gash(self.Cmax,self.EoverR,self.CanopyGapFraction, self.Precipitation)
    ...
    Transfer = min(self.UStoreDepth,ifthenelse (SaturationDeficit <= 0.00001, 0.0, Ksat * self.UStoreDepth/(SaturationDeficit+1)))
    ...
    self.FirstZoneDepth = self.FirstZoneDepth + Transfer - ActLeakage
    self.UStoreDepth = self.UStoreDepth - Transfer
    ...
    self.InwaterMM=max(0.0,self.ExfiltWater + FreeWaterDepth + self.SubCellRunoff + self.SubCellGWRunoff - Reinfilt)
    ...
    self.CumActInfilt = self.CumActInfilt + ActInfilt
