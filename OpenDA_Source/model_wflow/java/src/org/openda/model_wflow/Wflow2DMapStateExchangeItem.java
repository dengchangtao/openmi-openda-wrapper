...
//		return new TreeVector(getId(), vector, rowCount, columnCount);
	...
	public void copyValuesFromItem(IExchangeItem sourceItem) {
		...
		//get index of currentTime in sourceItem.
		double[] sourceTimes = sourceItem.getTimeInfo().getTimes();
		double currentTime = this.getTimeInfo().getTimes()[0];
		int currentTimeIndex = TimeUtils.findMatchingTimeIndex(sourceTimes, currentTime, 1e-5);
		if (currentTimeIndex == -1) {//if sourceItem does not contain currentTime.
			//no data to copy.
			return;
		}

		//get values for current time.
		double[] valuesForCurrentTime = ((NetcdfGridTimeSeriesExchangeItem) sourceItem).getValuesForSingleTime(currentTimeIndex);
		this.setAllValues(valuesForCurrentTime);
	}
