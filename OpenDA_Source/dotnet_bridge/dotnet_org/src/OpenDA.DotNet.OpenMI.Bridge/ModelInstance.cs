...
namespace OpenDA.DotNet.OpenMI.Bridge
{
	public class ModelInstance : IModelInstance
	{
	    ...
	    public void Compute(ITime targetTime)
		{
			...
			while (lastTime.EndTime.MJD < targetTime.MJD && _openMIComponent.Status != LinkableComponentStatus.Failed)
			{
			    try
			    {
        ...
        private void GetExchangeItemsFromOpenMIComponent()
        {
            ...
            foreach (IBaseInput baseInput in _openMIComponent.Inputs)
            {
                ...
                if (timeSpaceInput != null)
                {
                    ...
                    {
                        ModelFactory.AppendLogMessage("Input item " + timeSpaceInput.Id + " defined more then once");
                        errors = true;
                    }
                    else
                    {
            ...
            foreach (IBaseOutput baseOutput in _openMIComponent.Outputs)
            {
                ...
                if (timeSpaceOutput != null)
                {
                    ...
                    {
                        ModelFactory.AppendLogMessage("Output item " + timeSpaceOutput.Id + " defined more then once");
                        errors = true;
                    }
                    else
                    {
            ...
            if (errors)
               throw new Exception("Multiple Input and/or Output items");
        }
	}
}