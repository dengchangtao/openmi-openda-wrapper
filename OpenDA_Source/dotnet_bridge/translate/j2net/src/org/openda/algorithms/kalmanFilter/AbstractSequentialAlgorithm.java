...
public abstract class AbstractSequentialAlgorithm extends Instance implements IAlgorithm {
	...
	public IVector currentState = null;
	...
	public void finish() {
		...
		if (this.stochObserver != null) {
			this.stochObserver.free();
		}
	}
	...
	public void setStochComponents(IStochObserver stochObserver, IStochModelFactory stochModelFactory){
		...
		this.currentState = this.mainModel.getState();
        ...
        if (this.analysisTimes != null) {
            n = this.analysisTimes.length;
        } else {
            n = 0;
        }
		if ( n>0 ){
			if(this.finalTime.after(this.analysisTimes[n-1])){
        ...
        if (this.analysisTimes != null) {
		    this.maxSteps = this.analysisTimes.length;
        } else {
            if (!continueWithoutObservation) {
                throw new RuntimeException("No analysisTimes found in model time span " +
                        modelSpan.toString() + ", while continueWithoutObservation=false (default).");
	...
	public boolean hasNext() {
		return this.thisStep <= this.maxSteps;
	}
	...
	public void next() {
		...
		if (this.thisStep == this.maxSteps) { // final forecast
		...
		} else if (this.thisStep < this.maxSteps) { // more data to process
			...
			if (! OdaGlobSettings.getProductionRun()) {
				...
				Results.putValue("x_f_central", this.currentState, IResultWriter.MessageType.Step);
			...
			if(selection.getCount()>0 && ! this.skipAssimilation()){
				...
				Results.putProgression("========================================================================\n");
				Results.putValue("analysis_time", this.currentTime.getMJD(), IResultWriter.MessageType.Step);
				...
				Results.putValue("pred_f_central", pred_f, IResultWriter.MessageType.Step);
				IVector obsValues = selection.getExpectations();
				Results.putValue("obs", obsValues, IResultWriter.MessageType.Step);
				...
				if (! OdaGlobSettings.getProductionRun()) {
					...
					timerNext_getState.stop();
					timerNext_resultWriter.start();
					Results.putValue("x_a", currentState, IResultWriter.MessageType.Step);
				    timerNext_resultWriter.stop();
				}
				else {
				...
				timerNext_getObsVal.stop();
				Results.putValue("pred_a_central", pred_a, IResultWriter.MessageType.Step);
	...
	if (skipAtInitialTime){
		...
		if (skipAtFinalTime){
		   if (this.thisStep==this.maxSteps-1){
	...
	public void restoreInternalState(IModelState savedInternalState) {
		...
		this.thisStep=0;
		while((thisStep<this.maxSteps)&&(this.currentTime.after(this.analysisTimes[this.thisStep]))){
			this.thisStep++;
		}
