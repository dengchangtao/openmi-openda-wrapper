...
public class SimulationKwadraticCostFunction implements LeastSquaresCostFunction{
	...
	public void prepare(IVector p) {
        ...
        Results.putValue("evaluatedParameters", p, 1, "essential", "any", IResultWriter.MessageType.Instance);
        this.preparePars=p;
...
//            obsCost *= (1/(double)this.obs.getCount()); // normalized cost over nObs
	...
	public double evaluate(IVector p, String context) {
	    ...
	    if(this.useAnnounceObs){ // evaluate cost with one model-run
			...
			Results.putValue("predicted", prd, prd.getSize(), "normal", context, IResultWriter.MessageType.Instance);
            Results.putValue("observed", obsMean, obsMean.getSize(), "verbose", context, IResultWriter.MessageType.Instance);
            Results.putValue("residuals", residuals, residuals.getSize(), "verbose", context, IResultWriter.MessageType.Instance);
            Results.putValue("obs_stdev", obsStd, obsStd.getSize(), "all", context, IResultWriter.MessageType.Instance);
	    	// output statistics
	    	Statistics stats = new Statistics(residuals, descr);
            Results.putMessage(stats.toString());
...
//            obsCost *= (1/(double)this.obs.getCount()); // normalized cost over nObs
	    	totalCost += obsCost;
	    	Results.putValue("costObserved", obsCost, 1, "essential", context, IResultWriter.MessageType.Instance);
	    	Results.putValue("costTotal", totalCost, 1, "essential", context, IResultWriter.MessageType.Instance);
	    }else{ //without mod.announceObs, by splitting run in parts
	    	throw(new RuntimeException("Simulation without announceObs is not implemented yet"));
	    }
