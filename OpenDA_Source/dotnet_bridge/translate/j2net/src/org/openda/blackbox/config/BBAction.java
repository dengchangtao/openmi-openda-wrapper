...
public class BBAction {
    ...
    public void run(File instanceFileOrDir) {
        ...
        if (className != null) {
            try {
                returnValueAsObject = BBUtils.runJavaClass(aliasDefinitions.apply(className, aliasesUsedInClassName), instanceFileOrDir, getArguments());
            } catch (Exception e) {
            ...
            if (fullExePath == null) {
                ...
                fullExePath = BBUtils.determineExe(instanceFileOrDir, aliasDefinitions.apply(exePath, aliasesUsedInExeName));
            }
            try {
                File actualRunDir = instanceFileOrDir;
                if (actualWorkingDirectory != null) {
                    actualRunDir = new File(instanceFileOrDir,
							aliasDefinitions.apply(actualWorkingDirectory, aliasesUsedInActualWorkingDirectory));
                ...
                returnValueAsObject = BBUtils.runExecutable(fullExePath, actualRunDir, getArguments());
            } catch (Exception e) {
