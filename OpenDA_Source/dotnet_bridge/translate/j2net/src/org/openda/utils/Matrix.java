...
public class Matrix implements IMatrix {
   ...
   int n=0;
     ...
     * @param x updated vector in multiplication
     */
    public  void rightMultiply(double beta, IVector v, double alpha, IVector x){
       //TODO check for consistent dims
       for(int i=0;i<this.m;i++){
     ...
     * @param v updated vector in multiplication
     */
     ...
     */
    public Matrix(IVector v, IVector w){
    	this.m = v.getSize();
    	this.n = w.getSize(); 
    	this.values = new double[this.m][this.n];
    	for(int j=0;j<this.n;j++){
    		for(int i=0;i<this.m;i++){
	 			this.values[i][j] = v.getValue(i)*w.getValue(j);
 			}
 		}
    }
    /**
   ...
   public static Matrix mult(IMatrix A, IMatrix B){
	   Matrix result = Matrix.mult(A, B, false, false);
	   return result;
   }
