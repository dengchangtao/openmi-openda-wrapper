...
public class PowellCoreOptimizer{
	...
	void next(){
	    ...
	    if(this.moreToDo){
	    	...
	    	double f_prev = f_p;
            Results.putValue("p_start", p, IResultWriter.MessageType.OuterIteration);
            Results.putValue("f_p_start", f_p, IResultWriter.MessageType.OuterIteration);
		    ...
		    for (int i=0;i<n;i++){
	            ...
	            Results.putValue("searchDir", xi_i, IResultWriter.MessageType.InnerIteration);
		    	...
		    	p   = p_opt_i;
		    	f_p = f_opt_i;
                Results.putValue("p_dir", p, IResultWriter.MessageType.InnerIteration);
                Results.putValue("f_p_dir", f_p, IResultWriter.MessageType.InnerIteration);
            }
		    ...
		    if(this.moreToDo){
		    	...
		    	double fExtrapolate = this.f.evaluate(pExtrapolate);
	...
	private double LineSearch(IVector baseVector, IVector direction, IVector minVector){
		...
		for(int i=0;i<2;i++){
	        ...
	        fBracket[i] = this.f.evaluate(pTemp);
		}
