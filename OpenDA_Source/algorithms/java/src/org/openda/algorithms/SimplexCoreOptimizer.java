...
public class SimplexCoreOptimizer{
	...
	public void initialize(IVector pCurrent[]){
        ...
        for(int i=0;i<pCurrent.length;i++){
        	...
        	this.fCurrent[i] = this.f.evaluate(this.pCurrent[i]);
			if(this.f instanceof LeastSquaresCostFunction){
...
//		int imain=0;
        int flagstep;
	    Results.putValue("costs", new Vector(costs));
		if(this.moreToDo){
		      ...
		      cost_r     = this.f.evaluate(sreflect);
				if(this.f instanceof LeastSquaresCostFunction){
		      ...
		      if ((flagstep==0) & (cost_r<costs[0])){
		         ...
		         cost_e    = this.f.evaluate(sexpand);
		         if(this.f instanceof LeastSquaresCostFunction){
		      ...
		      if ((flagstep==0) & (cost_r>=costs[nparam-1]) & (cost_r<costs[nparam])){
		        ...
		        cost_c      = this.f.evaluate(scontract);
				if(this.f instanceof LeastSquaresCostFunction){
		      ...
		      if (flagstep==0){
		        ...
		        for(int ivertex=1;ivertex<nparam+1;ivertex++){
		          ...
		          cost_s             = this.f.evaluate(sshrink);
		          if(this.f instanceof LeastSquaresCostFunction){
		        ...
		        if (costmax<costs[nparam]){
		           Results.putValue("maximumCost", costs[nparam]);
		...
		if(!this.moreToDo){
			...
			if(this.f instanceof LeastSquaresCostFunction){
				...
				if(1==1){ //TODO replace with check for computation fo error estimate
					...
					Results.putValue("parameterErrorEstimateStd", stdWithStructure, IResultWriter.MessageType.OuterIteration);
					/*
