...
public class AnalysisLeastSquaresCost implements LeastSquaresCostFunction {
	...
	public double evaluate(IVector p) {
        ...
        //Results.putValue("evaluatedParameters", p, ResultWriter.MessageType.Instance);
	    ...
	    if(this.backGroundOn){
	    	...
	    	//Results.putValue("costWeakConstraintPenalty", backgroundCost, ResultWriter.MessageType.Instance);
	    }
	    ...
	    if(prd.getSize()<this.maxPrintSize){
	    	//Results.putValue("predicted", prd, ResultWriter.MessageType.Instance);
	    	//Results.putValue("observed", obsMean, ResultWriter.MessageType.Instance);
	    	//Results.putValue("residuals", residuals, ResultWriter.MessageType.Instance);
	    	//Results.putValue("obs_stdev", obsStd, ResultWriter.MessageType.Instance);
	    }
	    // Jo = sum_k factor ((obs_k - prd_k)/sigma_o)^2
	    ...
	    obsCost *= this.factor;
	    totalCost += obsCost;
	    //Results.putValue("costObserved", obsCost, ResultWriter.MessageType.Instance);
	    //Results.putValue("cost", totalCost, ResultWriter.MessageType.Instance);
	...
	public double evaluate(IVector p, String context) {
		//TODO: extend this method with some calls to Results.putValue() to make use of the 'context' information.
		return evaluate(p);
	}
