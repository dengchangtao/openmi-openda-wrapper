...
public class Simulation extends Instance implements IAlgorithm {
	...
	public void next(){
		...
		if(this.stochParameter){
			IVector p= model.getParameterUncertainty().createRealization();
			model.setParameters(p);
			Results.putValue("par", p, IResultWriter.MessageType.Instance);
		}
    	...
    	// print output
    	Results.putValue("pred", prd, IResultWriter.MessageType.Instance);
    	Results.putValue("obs", obs, IResultWriter.MessageType.Instance);
    	Results.putValue("std_obs", obsStd, IResultWriter.MessageType.Instance);
    	Results.putValue("residual", residuals, IResultWriter.MessageType.Instance);
    	...
    	if(model instanceof SimpleStochModelInstance){
    		...
    		for(int j=0;j<xStore.size();j++){
    			Results.putValue("t", tStore.elementAt(j));
    			Results.putValue("x", xStore.elementAt(j));
    		}
    	}
		model.finish();
	}
