/*
$URL: https://repos.deltares.nl/repos/openda/openda_1/public/trunk/core/native/src/cta/cta_model.c $
...
int CTAI_Model_PerformTimesteps(
   ...
   if (isBlocked) {
     ...
     retval=CTAI_ModelFac_SetBlock(data->hmodcl,hmodel,tstart,tstop);
     /* advance all childmodels */
     retval=CTAI_ModelFac_TimeStepAllModels(data->hmodcl,function);
     if (retval!=CTA_OK) {
...
int CTA_Model_Compute(
   ...
   if (ntimes>0){
      ...
      if (data->announced.times[0]<=tstart ||
         ...
         printf("Tstart= %g, first announced=%g\n",
            tstart,data->announced.times[0]);
         printf("Tstop= %g, last announced=%g\n",
            tstop,data->announced.times[ntimes-1]);
