/*
$URL: https://repos.deltares.nl/repos/openda/openda_1/public/trunk/core/native/src/cta/cta_functions.c $
$Revision: 3406 $, $Date: 2012-08-16 15:25:53 +0200 (do, 16 aug 2012) $
   ...
   if (data->refcount==0){
      ...
      if (data->libhandle) {
         ...
         if (dlclose(data->libhandle)){
            printf("Error in CTA_Func_Free:Cannot close library\n");
            printf("%s\n",dlerror());
         }
#endif
      }
...
//   xmlChar       *funcname =NULL;     /* name of the function */
   ...
   if (retval!=CTA_OK){
      printf("Error in CTAI_CreateFuncDynamicLib:Cannot create a new COSTA function\n");
      return CTA_NULL;
   }
   ...
   /* WINDOWS VERSION FOR LOADING A FUNCTION FROM A SHARED LIBRARY: */
     ...
     /* Open Dynamic link library */
      ...
      if (data->libhandle==NULL){
         printf("Error in CTAI_XML_CreateFunc:Cannot open library %s\n",library);
         #ifdef HAVE_DLFCN_H
            printf("error is: |%s| \n",dlerror());
         #else
            printf("GetLastError code is %d\n",GetLastError());
         #endif
         nerror++;
      }
      ...
      if (data->function == NULL){  /* 6: NAME_ */
       ...
       if (data->function==NULL){
          printf("WARNING in CTAI_XML_CreateFunc:Cannot load function %s\n",functionName);
          #ifdef HAVE_DLFCN_H
             printf("%s\n",dlerror());
          #else
             printf("GetLastError code is %d\n",GetLastError());
   ...
   #else
      printf("Warning: Cannot load functions dynamically\n");
      printf("Compile COSTA with support for Dynamic loading \n");
      nerror++;
   #endif
