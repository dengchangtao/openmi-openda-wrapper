 ...
 *
 * \param O cta_comm_world          Fortran communcator CTA_COMM_WORLD
 * \param O cta_comm_myworld        Fortran communcator CTA_COMM_MYWORLD
 * \param O cta_comm_master_worker  Fortran communcator CTA_COMM_MASTER_WORKER
 *
 */
#ifdef USE_MPI
CTAEXPORT int CTA_Par_CreateNewCreateGetComm(CTA_ModelClass modelCls, MPI_Comm *comm);
#endif

/** \brief Get a communicator for a process group by index 
 *
 * \param I indx  indx of communicator
 * \param O comm  MPI communicator
 ...
 */
#ifdef USE_MPI
CTAEXPORT int CTA_Par_GetAllCommByIndex(int indx, MPI_Comm *comm);
#endif
