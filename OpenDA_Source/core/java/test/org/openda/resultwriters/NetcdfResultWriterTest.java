...
public class NetcdfResultWriterTest extends TestCase {
	...
	private void doAlgorithmSteps(int num) {
		...
		if (num == 1) {
			...
			Results.putValue(IResultWriter.Source.Algorithm, "ctatv_from_model_1", x);
			Results.putProgression("Netcdf-export: TREEVECTOR from model ");

			double cost=0.1;
			Results.putValue(IResultWriter.Source.Algorithm, "cost", cost);
		...
		if (num == 2) {
			...
			Results.putValue(IResultWriter.Source.Algorithm, "ctavector_2", vec1);
		...
		if (num == 3) {
			...
			Results.putValue(IResultWriter.Source.Algorithm, "ctatv_A_3", v2cta_part1);
		...
		if (num == 4) {
			...
			Results.putValue(IResultWriter.Source.Algorithm, "ctatv_B_4", v2cta_tot);
		...
		if (num == 5) {
			...
			Results.putValue(IResultWriter.Source.Algorithm, "simple_tv_5", v2);
		...
		if (num == 6) {
			...
			Results.putValue(IResultWriter.Source.Algorithm, "simplevec_6", v3);
		...
		if (num == 7) {
			// export the Matrix
			Results.putValue(IResultWriter.Source.Algorithm, "simplematrix_7", Mat1);
