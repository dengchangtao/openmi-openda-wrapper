...
public class NetcdfDataObject implements IComposableDataObject {
	...
	private void readNetcdfFile() throws IOException  {
		...
		for (Variable variable : netcdfFile.getVariables()) {
			...
			if (!variable.getDataType().isNumeric()) {
				Results.putMessage("WARNING: " + getClass().getSimpleName()
						+ ": only reading of numeric data is supported. Data in variable "
	...
	public double[] readDataForExchangeItemForSingleTime(IExchangeItem item, int timeDimensionIndex, int timeIndex) {
		Variable variable = getVariableForExchangeItem(item);
		return NetcdfUtils.readDataForVariableForSingleTime(variable, timeDimensionIndex, timeIndex);
	}

	public void writeDataForExchangeItemForSingleTime(IExchangeItem item, int timeIndex, double[] values) {
		Variable variable = getVariableForExchangeItem(item);
		makeSureFileHasBeenCreated();
		NetcdfUtils.writeDataForVariableForSingleTime(this.netcdfFile, variable, timeIndex, values);
	}
