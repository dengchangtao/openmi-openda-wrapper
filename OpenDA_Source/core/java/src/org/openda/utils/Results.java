...
public class Results {
     ...
     */
    public static void putValue(String id, Object result) {
        // no result source specified, assume algorithm
        putValue(IResultWriter.Source.Algorithm, id, result);
    }
     ...
     * @param result the output item
     */
    public static void putValue(IResultWriter.Source source, String id, Object result) {
        for (ConfiguredResultWriter configuredResultWriter : configuredResultWriters) {
            if (configuredResultWriter.getResultWriterConfig().doWrite(source, id)) {
                configuredResultWriter.getResultWriter().putValue(IResultWriter.Source.Algorithm, id, result);
            }
        }
        for (IResultWriter resultWriter : resultWriters) {
            resultWriter.putValue(IResultWriter.Source.Algorithm, id, result);
        }
    }
     ...
     * @param result the output item
     */
    public static void putValue(IInstance source, String id, Object result) {
        for (ConfiguredResultWriter configuredResultWriter : configuredResultWriters) {
            if (configuredResultWriter.getResultWriterConfig().doWrite(source, id)) {
                configuredResultWriter.getResultWriter().putValue(source, id, result);
            }
        }
        for (IResultWriter resultWriter : resultWriters) {
            resultWriter.putValue(source, id, result);
        }
    }
    /**
     * Put a vector, indicating the algorithm's current step type (iteration, time step, etc)
     *
     * @param id     the output vector's identifier
     * @param result the output item
     * @param type   result type (iteration, timestep, etc.)
     */
    public static void putValue(String id, Object result, IResultWriter.MessageType type) {
        IResultWriter.Source source = IResultWriter.Source.Algorithm;
        for (ConfiguredResultWriter configuredResultWriter : configuredResultWriters) {
            int writeCounter = configuredResultWriter.getResultWriterConfig().nextWriteCounter(source, id, type);
            if (writeCounter >= 0) {
                configuredResultWriter.getResultWriter().putValue(source, id, result, writeCounter);
                checkCounter(type, writeCounter);
            }
        }
        for (IResultWriter resultWriter : resultWriters) {
            resultWriter.putValue(IResultWriter.Source.Algorithm, id, result, determineCounter(type));
        }
    }

	public static void putValue(String id, Object result, int size, String outputLevel, String context,
								IResultWriter.MessageType type) {
		//TODO: remove source
		IResultWriter.Source source = IResultWriter.Source.Algorithm;
		for (ConfiguredResultWriter configuredResultWriter : configuredResultWriters) {
			int writeCounter = configuredResultWriter.getResultWriterConfig().nextWriteCounter(id, size,
					outputLevel, context, type);
			if (writeCounter >= 0) {
