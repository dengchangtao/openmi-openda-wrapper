...
public class UncertaintyEngine implements Serializable{
    ...
    private HashMap<String, String> uncertaintyRegexMapping = null;
    ...
    public void startCheckingUncertainItems() {
        uncertaintyRegexMapping = new HashMap<String, String>();
    ...
    public double[] getStandardDeviations(List<String> uncertaintyIds, HashMap<String, Integer> valueSizes) {
        return getStandardDeviations(uncertaintyIds, valueSizes, null);
    }

    public double[] getStandardDeviations(List<String> uncertaintyIds,
                                          HashMap<String, Integer> valueSizes,
                                          double[] actualValues) {

        int size = 0;
        boolean allSizesAreOne=true;
        if (valueSizes == null || valueSizes.size() == 0) {
            // no sizes specified, one value per uncertainty id
            size = uncertaintyIds.size();
        } else {
            // individual sizes specified, determine total
            for (Integer valueSize : valueSizes.values()) {
                size += valueSize;
            }
            allSizesAreOne = false;
        }
        ...
        for (String uncertaintyId : uncertaintyIds) {
            ...
            if (stdDevIsFactor && actualValues == null) {
                    throw new RuntimeException(this.getClass() +  "getStandardDeviations(): Uncertainty " +
                            " is defined as factor, but no actual values is provided");
            }
            if (allSizesAreOne) {
                stdDevs[index] = stdDevIsFactor ? Math.abs(stdDev*actualValues[index]) : stdDev;
                index++;
            } else {
                for (int i = 0; i < valueSizes.get(uncertaintyId); i++) {
                    stdDevs[index] = stdDevIsFactor ? Math.abs(stdDev*actualValues[index]) : stdDev;
                    index++;
                }
            }
    ...
    public IVector getRealization(IVector values) {
        if (values instanceof ITreeVector) {
            ...
            String[] uncertaintyIDs = getUncertaintyIDs();
