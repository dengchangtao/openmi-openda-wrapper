 ...
 * - The property 'standardDeviation' of each TimeSeries gived the spread of the errors
...
public class TimeSeriesStochObserver extends Instance implements IStochObserver {
	...
	public IVector getStandardDeviations() {
		...
		int seriesLength;
		double stdSeries;
		String stdSeriesString;
		for(int i=0;i<this.series.length;i++){
			double[] valuePart = this.series[i].getValuesRef();
			if(valuePart!=null){
				seriesLength = valuePart.length;
				stdSeriesString    = this.series[i].getProperty("standarddeviation");
				if(stdSeriesString==null){
					throw new RuntimeException("No standardDeviation was defined for series nr. "+i+"(first series has index 0)");
				}
				try {
					stdSeries = Double.parseDouble(stdSeriesString);
				} catch (Exception e) {
					throw new RuntimeException("Could not parse standardDeviation for series nr. "+i+
							"String to parse is "+stdSeriesString);
				}
				// set values
				for(int j=0;j<seriesLength;j++){
					values[nextindex] = stdSeries;
					nextindex++;
				}
			}
		}
